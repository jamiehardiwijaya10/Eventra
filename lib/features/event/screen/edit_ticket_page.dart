...
class _EditTicketPageState extends State<EditTicketPage> {
  ...
  late final TextEditingController maxPerUserController;
  ...
  void initState() {
    ...
    maxPerUserController = TextEditingController(
      text: ticket.maxPerUser.toString(),
    );
  ...
  Future<void> selectSaleStart() async {
    ...
    final time = await showTimePicker(
      context: context,
      initialTime: saleStart != null
          ? TimeOfDay.fromDateTime(saleStart!)
          : TimeOfDay.now(),
    );
  ...
  Future<void> selectSaleEnd() async {
    ...
    final time = await showTimePicker(
      context: context,
      initialTime: saleEnd != null
          ? TimeOfDay.fromDateTime(saleEnd!)
          : TimeOfDay.now(),
    );
  ...
  Future<void> loadTicketDetails() async {
    try {
      final data = await _ticketService.getTicketById(
        widget.ticket.id,
      );
      ...
      setState(() {
        if (data['sale_start'] != null) {
          saleStart = DateTime.parse(
            data['sale_start'].toString(),
          ).toLocal();
        }

        if (data['sale_end'] != null) {
          saleEnd = DateTime.parse(
            data['sale_end'].toString(),
          ).toLocal();
        }
  ...
  Future<void> updateTicket() async {
    ...
    final maxPerUser =
    int.tryParse(maxPerUserController.text.trim());
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const Text(
              'Update the configuration of this ticket.',
              style: TextStyle(
                color: Colors.grey,
              ),
            ),
            ...
            const Text(
              'On Sale, Sold Out, and Ended are '
